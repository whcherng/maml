#!/usr/bin/env python3
"""
Iteration V5 — Sigmoid Position Mapping + Increased Training

Expected: Sharpe ≈ 1.037, Total Return ≈ 26.71%

Changes from V3 (V4 was reverted):
  - Sigmoid position mapping: 1/(1+exp(-8*(p_long-0.5)))
  - meta_epochs=200, tasks_per_batch=12
  - Vol-scaling removed (reverted from V4)
"""
...
class V5DataPipeline(DataPipeline):
...
class V5Backtester:
    """V5: sigmoid position mapping, no vol-scaling."""
    ...
    def _signal_to_position(self, prediction, probs):
        """V5: sigmoid mapping centered at 0.5."""
        p_long = float(probs[1])
        steepness = 8.0
        position = 1.0 / (1.0 + np.exp(-steepness * (p_long - 0.5)))
...
def main():
    config = MAMLConfig(
        ...
        tasks_per_batch=12, k_shot=20, query_size=20,
    ...
    print(f"[V5] Device: {config.device}")

    pipeline = V5DataPipeline(config)
    processed_data = pipeline.run()
    feature_columns = pipeline.get_feature_columns()
    config.input_dim = len(feature_columns)
    print(f"[V5] Feature dimension: {config.input_dim}")

    backtester = V5Backtester(config, processed_data, feature_columns)
