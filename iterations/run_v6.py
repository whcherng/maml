#!/usr/bin/env python3
"""
Iteration V6 — Ensemble Inference + Trade Optimization (Final Best)

Expected: Sharpe ≈ 1.834, Total Return ≈ 62.15%

Changes from V5:
  - Ensemble inference: 5 adaptations with shuffled support sets, averaged
  - Sigmoid center shifted from 0.50 to 0.45 (bull-market bias)
  - Minimum trade threshold: 8% position change
  - k_shot increased from 20 to 30

This is equivalent to running `python main.py` with the final codebase.
"""
...
class V6DataPipeline(DataPipeline):
...
class V6Backtester:
    """V6: ensemble inference, bull-biased sigmoid, min trade threshold."""
    ...
    def _signal_to_position(self, prediction, probs):
        """V6: sigmoid with bull-bias center=0.45."""
    ...
    def run(self, meta_train_epochs=50, verbose=True):
        ...
        if verbose:
            print(f"\n[V6 Backtester] Split: train={train_end}, "
        ...
        # V6 uses the main MAMLTradingEngine which has ensemble inference
        ...
        for ticker, df in self.processed_data.items():
            ...
            for t in range(embargo_end + seq_len + k_shot, test_end):
                ...
                # V6: ensemble inference (n_ensemble=5 is default)
...
def main():
    config = MAMLConfig(
        ...
        # V6: final tuned hyperparameters
    ...
    print(f"[V6] Device: {config.device}")

    pipeline = V6DataPipeline(config)
    processed_data = pipeline.run()
    feature_columns = pipeline.get_feature_columns()
    config.input_dim = len(feature_columns)
    print(f"[V6] Feature dimension: {config.input_dim}")

    backtester = V6Backtester(config, processed_data, feature_columns)
