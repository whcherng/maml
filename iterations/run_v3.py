#!/usr/bin/env python3
"""
Iteration V3 — Bull-Bias Position Sizing + Equal-Weight Portfolio

Expected: Sharpe ≈ 0.404, Total Return ≈ 9.55%

Changes from V2:
  - Graduated position sizing with bull-market bias (0.0/0.3/0.6/1.0)
  - Equal-weight portfolio aggregation (not sequential concat)
  - Cross-sectional training (ONE model across ALL tickers)
  - Curated 11 features (down from 29)
  - Softened risk management (-20% DD, 5-day cooldown, 30% reduction)
  - Faster position smoothing (alpha=0.7, floor=0.03)
  - inner_steps reduced to 3
"""
...
class V3DataPipeline(DataPipeline):
    """V3: binary labels, curated 11 features."""
    ...
    def get_feature_columns(self):
        """V3: curated 11 features."""
        curated = [
            "SMA_Cross", "RSI", "Mom_5d", "Mom_20d", "BB_width",
...
class V3Backtester:
    """V3: cross-sectional training, EW portfolio, graduated sizing."""
    ...
    def _signal_to_position(self, prediction, probs):
        """V3: graduated bull-bias position sizing."""
    ...
    def run(self, meta_train_epochs=50, verbose=True):
        ...
        if verbose:
            print(f"\n[V3 Backtester] Split: train={train_end}, "
                  f"val={val_end}, embargo={embargo_end}, test={test_end}")
...
def main():
    ...
    print(f"[V3] Device: {config.device}")

    pipeline = V3DataPipeline(config)
    processed_data = pipeline.run()
    feature_columns = pipeline.get_feature_columns()
    config.input_dim = len(feature_columns)
    print(f"[V3] Feature dimension: {config.input_dim}")

    backtester = V3Backtester(config, processed_data, feature_columns)
